...
### 게시글 [목록] 버튼 클릭 시 올바른 게시판으로 이동 안 되는 버그 수정

**수정 파일**: `src/composables/usePostDetail.js`

**MVVM 레이어**: ViewModel

`usePostDetail.js`는 `src/composables/` 폴더 안에 있고 이름이 `use***` 형태이므로 ViewModel에 해당합니다.

이번에 수정한 내용은 "지금 어느 게시판에 있는지"를 저장하고 관리하는 로직입니다.
화면에 표시하는 것도 아니고, 서버에서 데이터를 가져오는 것도 아닙니다.
값을 계산하고 저장하는 역할이므로 ViewModel인 `usePostDetail.js`에 두는 것이 맞습니다.

각 레이어의 역할 분담:

| MVVM 레이어 | 파일 | 이번 수정에서의 역할 |
|---|---|---|
| **View** | `PostDetailView.vue` | 화면에 게시글 내용을 보여주는 역할 |
| **ViewModel** | `usePostDetail.js` | 현재 게시판이 어디인지 계산하고 저장, [목록] 버튼 클릭 시 어디로 이동할지 결정 ← 수정된 곳 |
| **Model** | `api.js` | 서버에서 게시글 데이터를 실제로 불러오는 역할 |

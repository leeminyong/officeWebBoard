...
// parentKey : 하위 메뉴로 추가할 때 부모 게시판 key입니다. 없으면 null(최상위 메뉴).
// 반환값: { ok: true/false, data: { key, label, parent_key } 또는 { error: '...' } }
export async function addBoard(label, parentKey = null) {
  const res = await fetch('/api/boards', {
    method: 'POST',
    ...
    // parentKey 가 null 이면 서버에서도 NULL로 저장되어 최상위 메뉴가 됩니다.
    body: JSON.stringify({ label, parentKey }),
  })
